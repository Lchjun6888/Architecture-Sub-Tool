...
const RuleConfigurator = ({ rowsPerFile, setRowsPerFile, keywords, setKeywords, headerRows, setHeaderRows, onAddKeyword, onRemoveKeyword, isProPlan }) => {
    return (
            ...
            <div className="space-y-6">
                <div>
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <input
                            type="number"
                            min="1"
                            max={isProPlan ? undefined : 500}
                            value={rowsPerFile}
                            onChange={(e) => {
                                let val = parseInt(e.target.value) || 1;
                                if (!isProPlan && val > 500) {
                                    alert("Free plan is limited to 500 rows per file. Please upgrade to Pro.");
                                    val = 500;
                                }
                                setRowsPerFile(val);
                            }}
                        ...
                        />
                        {!isProPlan && <span className="text-xs text-slate-400 mt-1 block text-right">Max 500 (Free)</span>}
                    </div>
                </div>
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 relative">
                        {!isProPlan && (
                            <div className="absolute inset-0 z-10 bg-white/50 dark:bg-slate-800/50 backdrop-blur-[1px] rounded-lg flex flex-col items-center justify-center cursor-not-allowed">
                                ...
                                <span className="text-xs font-bold text-slate-600 dark:text-slate-300 text-center">Smart Keyword Split</span>
                            </div>
                        )}
                        <div className={`grid grid-cols-1 sm:grid-cols-2 gap-2 flex-1 ${!isProPlan ? 'opacity-30' : ''}`}>
                            {keywords.map((keyword, index) => (
                                <div key={index} className="relative group">
                                    <input
                                        ...
                                        disabled={!isProPlan}
                                    />
                                    <button onClick={() => isProPlan && onRemoveKeyword(index)} className="absolute right-1.5 top-1.5 text-slate-400" disabled={!isProPlan}><X size={16} /></button>
                                </div>
                            ))}
                        </div>
                        <button onClick={() => isProPlan && onAddKeyword()} className={`w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 ${!isProPlan ? 'opacity-30 cursor-not-allowed' : ''}`} disabled={!isProPlan}><Plus size={24} /></button>
                    </div>
                </div>
